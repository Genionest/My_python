...
def get_areas(url):
    ...
    responsne = requests.get(url,headers=headers)
    content = etree.HTML(responsne.text)
    areas = content.xpath("//dd[@data-index = '0']//div[@class='option-list']/a/@href")
...
def get_pages(area,area_link):
    ...
    responsne = requests.get(area_link,headers=headers)
    ...
    try:
        ...
        for i in range(30):
            ...
            print('writing work has done!contiune the next page')

    except Exception as e:
        print('ooops! connecting error, retrying...')
        time.sleep(20)
        return get_huse_info(area,url)
